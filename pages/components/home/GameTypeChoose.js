...
const GameTypeChoose = () => {
  return (
    ...
    <section className="w-full lg:h-[630px] lg:grid bg-no-repeat md:flex md:flex-col grid-cols-3 bg-cover bg-bottom">
      {GAMETYPE.map((type, index) => {
        return (
          <article
            key={index}
            // className={`h-full flex flex-col items-center py-[50px] lg:!bg-none bg-cover`}
            className={`h-full flex justify-center flex-col items-center py-[50px] bg-cover`}
            style={{ backgroundImage: `url(/Images/gameType/${type.t}2.jpg)` }}
          >
